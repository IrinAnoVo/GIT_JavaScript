...
function renderModalWindowContent(productId) {
  ...
  /*modalThumbnails.innerHTML = product.images.map((src, index) => `<img class="gallery-thumb ${index === currentImageIndex ? 'active'  : ''}" src="${src}" alt="${product.title}" data-index="${index}>`).join("");*/
  ...
  const thumbs = document.querySelectorAll(".gallery-thumb")
   thumbs.forEach(thumb => {
    thumb.addEventListener("click", function () {
       /*currentImageIndex = +this.dataset.thumb;*/

       mainModalImage.innerHTML = `<img src="${this.src}" alt="${this.alt}">`;
